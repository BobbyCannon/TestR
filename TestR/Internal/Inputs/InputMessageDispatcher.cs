...
namespace TestR.Internal.Inputs
{
	...
	internal class InputMessageDispatcher
	{
		...
		/// <summary>
		/// Dispatches the specified list of <see cref="Input" /> messages in their specified order by issuing a single called to <see cref="NativeInput.SendInput" />.
		/// </summary>
		...
		public void DispatchInput(Input[] inputs)
		{
			...
			var successful = NativeInput.SendInput((uint) inputs.Length, inputs, Marshal.SizeOf(typeof(Input)));
